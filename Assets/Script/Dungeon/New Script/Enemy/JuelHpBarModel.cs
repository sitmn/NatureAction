...
public class JuelHpBarModel : MonoBehaviour
{
    ...
    public void SetHpUI(int _hp, int _juelNo)
    {
        SetHpBar(_hp, _juelNo);
    }

    //HPBarの変動
    private void SetHpBar(int _hp, int _juelNo)
    {
        float _hpRaito = (float)_hp / (float)EnemyStatus._juelStatusData.sheets[0].list[_juelNo]._juelMaxHp;
